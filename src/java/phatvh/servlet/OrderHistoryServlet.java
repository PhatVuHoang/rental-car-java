...
public class OrderHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArrayList<orderDTO2> listOrderDetail = dao.getOrderHistoryDetail(email);
            ArrayList<orderDTO2> listOrder = dao.getOrder(email);
            session.setAttribute("LISTORDER", listOrder);
            session.setAttribute("LISTORDERTEMP", listOrder);
            session.setAttribute("LISTORDERDETAIL", listOrderDetail);
            session.setAttribute("SELECTORDERSEARCH", "1");
            session.setAttribute("SEARCHORDERNAME", "");
        } catch (SQLException e) {
            log("SQLException_OrderHistoryServlet: " + e.getMessage());
        } catch (NamingException e) {
            log("NamingException_OrderHistoryServlet: " + e.getMessage());
        }
        finally {
