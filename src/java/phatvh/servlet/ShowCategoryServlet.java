...
import phatvh.category.categoryDAO;
...
public class ShowCategoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ArrayList<categoryDTO> listCategory = dao.getCategory();
            session.setAttribute("CATEGORY", listCategory);
            session.setAttribute("SELECT", 1);
        } catch(SQLException e) {
            log("SQLException_ShowCategoryServlet: " + e.getMessage());
        } catch(NamingException e) {
            log("NamingException_ShowCategoryServlet: " + e.getMessage());
        }
        finally {
